...
Course_packages <- c("tidyverse", "rjson", "tidyjson", "readr", "DT", "gapminder", "gganimate"  )
...
```
Join de las dos tablas de contaminación anteriores.
```{r , eval = TRUE,message=FALSE, warning=FALSE}
# Convertir las tablas de formato ancho a formato largo
PM2_ccaa_media_long <- PM2_ccaa_media %>%
  pivot_longer(
    cols = c("2022", "2021", "2020", "2019", "2018", "2017", "2016", "2015",
...
media_calidad_aire <- calidad_aire %>%
...
ggplot(media_calidad_aire, aes(gdpPercap, lifeExp, size = pop, colour = country)) +
